...
В вывод команды show ip АРМов PC-A и PC-B будут отображены настройки:
...
Необходимо создать и задать имя VLAN в соответствии с таблицей VLAN, назначить ip адрес на vlan управления, назначить шлюз по умолчанию, все неиспользуемые порты перевести в ражим access, назначить в парковочный VLAN и отключить.
...
Необходимо настроить транковый режим портов на коммутаторах в сторону друг друга и в сторону маршрутизатора, настроить 8 VLAN как native vlan, назначить в качестве разрешенных к передаче vlan 3, 4 и 8.   
...
## Часть 5. Выполнение
 В части 5 необходимо произвести ping запросы для проверки связи между узлами:
