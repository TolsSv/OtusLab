...
Необходимо настроить на маршрутизаторе R1 DHCPv6 без сохранения состояния, для этого необходимо создать dhcp пул в котором необходимо настроить other-config-flag, назначить в качестве dns сервера 2001:db8:acad::254 и доменное имя STATELESS.com. Далее необходимо назначить пул на интерфейс e0/1 (GigabitEthernet 0/0/1).
...
Для настройки ретрансляции необходимо настроить managed-config-flag на интерфейсе e0/1 (GigabitEthernet 0/0/1) маршрутизатора R2 и включить dhcpv6 ретрансляцию в сторону маршрутизатора R1 за портом g0/0/0.

При настройке dhcpv6 ретрансляции произошла ошибка, видимо cisco packet tracer не поддерживает dhcpv6 ретрансляцию. Дальнейшая настройка и проверка корректности работы невозможна.
